...
def moves(tankPosJ, tankHealthJ, movesJ, field):
    ...
    while not (movesImpacted[0] and movesImpacted[1]):
        ...
        shellLocJ = ((dX(cTime, vectorMovesJ[0][0]) + tankPosJ[0][0],
                      dY(cTime, vectorMovesJ[0][1]) + tankPosJ[0][1]),
                     (dX(cTime, vectorMovesJ[1][0]) + tankPosJ[1][0],
                      dY(cTime, vectorMovesJ[1][1]) + tankPosJ[1][1]))

        for i in range(2):
            if not movesImpacted[i]:
                if impacted(shellLocJ[i], field):
                    field, tankPosJ, tankHealthJ = impact.applyImpact(
                        shellLocJ[i], field, tankPosJ, tankHealthJ)
                    #print(tankPosJ)
                    movesImpacted[i] = True

        shellLocJint = (tuple(
            (tuple((int(x) for x in shellLoc)) for shellLoc in shellLocJ)))
...
def vectorize(move):
    return (POWER_MULTIPLIER * move[0] * math.cos(math.radians(move[1])),
            POWER_MULTIPLIER * move[0] * math.sin(math.radians(move[1])))
